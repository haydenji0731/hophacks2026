...
Spoken phrases are frozen in `clap_phrases.json` (same idea as the news wire). CLAP reads that file on each clip; it does **not** query Postgres on the hot path. Refresh starts from the built-in seed (`gift card`, `IRS`, `arrest warrant`, …) then adds unused spoken strings from `scams` (short description clauses and mapped demands — not pattern slugs or `cash`/`other`).
...
`GET /v1/phrases` — current frozen book (what CLAP scores). `POST /v1/phrases/refresh` — rewrite the JSON. Restart uvicorn is not required; the detector reloads the file within a minute.
